...
class UpdateDialog:
    ...
    def install_update(self, download_path: str):
        ...
        if apply_update(download_path):
            self.title_label.configure(text="✅ Update Ready!")
            self.status_label.configure(
                text="The updater will run after this app closes.\nPlease relaunch the app when prompted.",
                text_color=TEXT_SECONDARY
            )
            # Give the user a moment to read the message, then force-exit so
            # the EXE file lock is released and the batch script can rename it.
