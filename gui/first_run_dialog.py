...
def _check_chromium() -> tuple[bool, str]:
    ...
    # Fast path: check persistent browsers directory
    ...
    # Slow path: ask Playwright directly
    try:
...
class FirstRunDialog:
    ...
    def _install_chromium(self):
        ...
        def _worker():
            try:
                ...
                # Pin the install location to our persistent browser dir
                env = os.environ.copy()
    ...
    def _install_success(self):
        ...
        self.dialog.destroy()
        # Re-spawn the dialog so the prereq check re-runs with updated Chromium status
        from gui.first_run_dialog import show_first_run_dialog
        show_first_run_dialog(parent, on_complete)
